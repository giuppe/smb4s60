...
class NetBIOS:
    ...
    def gethostbyname(self, nbname, type = TYPE_WORKSTATION, scope = None, timeout = 2):
    ...
    def getnodestatus(self, nbname, destaddr = None, type = TYPE_WORKSTATION, scope = None, timeout = 2):
        if destaddr:
    ...
    def __queryname(self, nbname, destaddr, type, scope, timeout):
        ...
        while 1:
            try:
				#print "entering loop %s" % (tries,)
				#e32.ao_sleep(.5)
				#print "After .5 sleep"
				#if not self.__sock._recv_will_return_data_immediately():
				#	print "when data not yet available"
				#	e32.ao_sleep(timeout)
				#	print "After sleeping %s seconds" % (timeout,)
				#if not self.__sock._recv_will_return_data_immediately():
				#	print "when data not yet available"
				...
				if 1:
#					print "in else part: %s" % (self.__sock._recv_will_return_data_immediately())
    ...
    def __querynodestatus(self, nbname, destaddr, type, scope, timeout):
        ...
        while 1:
            try:
                self.__sock.sendto(req, 0, ( destaddr, self.__servport ))
#				e32.ao_sleep(.5)
 #               ready = self.__sock._recv_will_return_data_immediately()
  #              if not ready:
	#				e32.ao_sleep(timeout)
	#				ready = self.__sock._recv_will_return_data_immediately()
	#			if not ready:
...
class NetBIOSSession:
    ...
    def recv_packet(self, timeout = 2):
    ...
    def __request_session(self, host_type, timeout = 2):
